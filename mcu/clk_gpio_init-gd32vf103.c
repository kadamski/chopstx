...
clock_init (void)
{
  ...
  /* CK_ADC = CK_APB2/8 */
  RCU->CFG0 |= RCU_CFG0_ADC_CKAPB2_DIV8;

  /* CK_TIMER1xxx, CK_TIMER0, CK_I2S */

  /* PLL setup */
  RCU->CFG0 &= ~(RCU_CFG0_PLLSRC_PLLMF_MASK|RCU_CFG0_PREDV0_LSB);
  RCU->CFG0 |= RCU_CFG0_PLL_MUL_VALUE | RCU_CFG0_PLLSRC_HXTAL;

  RCU->CFG1 &= ~(RCU_CFG1_PREDV0SEL_MASK|RCU_CFG1_PREDV1_PREDV0_MASK);
